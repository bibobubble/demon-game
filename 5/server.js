...
function forceReset() {
        if(confirm("确定要重置房间吗？这会踢出所有正在玩的玩家！")) {
            socket.emit('forceReset');
            alert("房间已重置，请再次尝试点击'进入房间'");
        }
    }
...
// --- 新增：强制重置房间 (用于解决卡死) ---
    socket.on('forceReset', () => {
        console.log('收到强制重置指令');
        // 清空所有数据
        gameState.players = [];
        gameState.demon = { r: null };
        gameState.shards = [];
        gameState.turnIdx = 0;
        gameState.phase = 'LOBBY'; // 关键：回到大厅状态
    ...
    socket.on('disconnect', () => {
        ...
        if (pIndex !== -1) {
            ...
            if(gameState.phase === 'LOBBY') {
                ...
                // 如果游戏已开始，标记为离线
                // 但如果所有人都断线了，就重置房间！
                // 我们通过查看 io.engine.clientsCount 来判断当前连接数
                setTimeout(() => {
                     // 延迟一点点检查，防止只是刷新网页
                     if (io.engine.clientsCount === 0) {
                         console.log("房间空了，自动重置...");
                         gameState.players = [];
